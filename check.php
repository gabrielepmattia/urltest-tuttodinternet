...
if($_POST['indirizzi']){
?>
<table cellpadding="4px;" style="margin:0; border:0;">
 <tr style="border-bottom:solid 2px #000; font-weight:bold">
  <td>Link</td>
  <td>Code</td>
  <td>Response</td>
  <td>Notes</td></tr>
...
$fp2 = fopen($nomerand,"r"); // Riapro lo stesso file in lettura
 ...
 if($fp2){
  ...
  file_put_contents($req_old_link,$req_old + 1);
	 ...
	 if ($counter == ($max_urls_allow + 1)) { break; }
	   // Check blank line
	   $riga = trim($riga);
	   if($riga == "") continue;
	   $counter++;
	  ...
	  if($headers){ // Assicuro che si riescano ad ottenere gli header
		  ...
		  // Controllo il codice errore e setto i colori
			 if($codice < 299){ $color = "1AD904";};
				 if($codice < 399 && $codice > 299){$color = "FF6600";};
				 if($codice >= 399){$color = "FF0000";};
				 if($codice == 301){$notes = "Redirect to: " . substr($headers[6],10);};
				 if($codice == 404){$notes = "Dead link";};				  
...
echo '<tr style="background-color:#'. $color .';">
		...
		<td class="notes">'		. $notes .'</td>
		 ...
		 if(trim($riga) != ""){$err = $riga; $color_blank = "FF0000"; $notes_blank = "Hostname not found";}; // controllo se effettivamente la riga del link è vuota e se non lo è setto il colore rosso e l'output della riga stessa
 ...
 } else {echo "Error access temporary memory area. Please contact the webmaster.";}
...
<?php } else { echo "You put no urls<br /><br />";} 
/* MULTIPLE URLs CHECKER - script by gabry3795 (c) 2014
All rights reserved. gabry.gabry@hotmail.it */ 
?>
